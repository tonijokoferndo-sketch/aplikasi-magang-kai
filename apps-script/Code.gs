  ...
  try {
    ...
    if (e && e.postData && e.postData.type && e.postData.contents) {
      try {
        ...
        // If contents is not JSON, fall back to parameters below
        payload = {};
      }
    }

    // If no payload from JSON body, try form parameters (e.parameter)
    if ((!payload || Object.keys(payload).length === 0) && e && e.parameter) {
      for (const k in e.parameter) {
        try { payload[k] = JSON.parse(e.parameter[k]); } catch (ex) { payload[k] = e.parameter[k]; }
      }
    }
    ...
    return Helper.output(Helper.sendResponse(false, 'Server error: ' + error.toString()));
  }
}
